...
<tbody>
  <?php if ($users) : ?>
    ...
    <tr class="<?php echo ($index%2 == 0) ? 'zebra' : 'cross'?>">
        ...
        <td> <?=($user->status == 1) ? anchor('users/deactive/'.$user->id, '<i class="icon-thumbs-up"></i> Active', array('onclick' => "return confirm('Do you want to deactive this data?')")) : anchor('users/active/'.$user->id, '<i class="icon-thumbs-down"></i> Deactive', array('onclick' => "return confirm('Do you want to active this data?')"))?> </td>
